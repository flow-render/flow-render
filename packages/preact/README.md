...
Render [Preact](https://preactjs.com/) components in asynchronous flows — a Promise-based UI rendering approach that lets you naturally await
user interactions, just like calling async functions.

## Installation

```bash
npm i @flow-render/preact
```
...
```tsx
import { Viewport } from '@flow-render/preact'
...
```tsx
import { type PromiseResolvers } from '@flow-render/preact'
...
```tsx
import { render } from '@flow-render/preact'
